...
and \gls{MOOSE} \cite{gaston_physics-based_2015}. Researchers at Politecnico di
...
this work, and Rattlesnake \cite{wang_rattlesnake_2021}. The MOOSE
framework facilitates multiphysics coupling among all MOOSE-based applications
by providing a common framework for shared data access and file
input/output, thus eliminating computational costs from data transfers and
optionally allowing for fully coupled solves. Abou-Jaoude et al.
...
As mentioned in Section \ref{sec:msr-tools}, Moltres is a multiphysics

Similarly, Moltres benefits from the
highly-integrated cross compatibility within the ecosystem of MOOSE-based
applications. Moltres solves the multigroup neutron diffusion equations, along
...
\texttt{Navier-Stokes} module for incompressible flow with the Boussinesq
approximation for buoyancy-driven flow.

MOOSE-based applications adopt
the object-oriented programming style in \texttt{C++} to modularize various
functions relevant to a finite-element solver. In this approach, MOOSE-based
applications break down \glspl{PDE} into individual terms and store them
...
\gls{PDE}. Object-oriented programming in MOOSE also helps software developers
reduce development time since new \texttt{kernels} can inherit common functions
from existing \texttt{kernels} which share similar physics.
