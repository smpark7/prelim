...
these libraries for their \gls{PDE} solving capabilities. MOOSE supports
modeling on up to three-dimensional (3-D) meshes for a wide range of mesh file
formats, including the commonly used Exodus II file format. Specifically for
2-D meshes, users can opt between Cartesian and polar coordinates. 

Moltres also benefits
from the highly-integrated cross compatibility within the ecosystem of
MOOSE-based applications.
MOOSE facilitates multiphysics coupling among all MOOSE-based applications
...
optionally allowing for fully coupled solves.
...
flow with the Boussinesq approximation for buoyancy-driven flow.
