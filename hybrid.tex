...
region as the $S_N$ \textit{subproblem} or \textit{sub-solver}. I define the entire problem
...
\end{align}
%
These boundary conditions for the $S_N$ sub-solver will generally yield inaccurate flux
...
generated with the $S_N$ sub-solver within $\Omega^d_1$ will be accurate everywhere except near
...
The $S_N$ sub-solver only covers $\Omega^d_1$, so it does not calculate an independent $k$
