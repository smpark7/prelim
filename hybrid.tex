...
domain. These diffusion coefficients are distinct from $P_1$-based diffusion coefficients,
which can go by the same name.
...
compatible quadrature set to replace the necessary integrals across the continuous solid angles
...
the neutron transport equation with isotropic sources. Essentially, a single value represents the
isotropic diffusion coefficient for an entire subregion (e.g., molten salt, graphite, or reflector
regions). However, as discussed in Section \ref{sec:challenges-control-rod}, the neutron diffusion
equation is only valid in regions of high scattering-to-removal ratios, away from interfaces
to neighboring media with highly dissimilar neutronic properties, at most linearly anisotropic
scattering, and small flux gradients.
...
coefficient variable that varies in space, even within a subregion. It provides pointwise
...
neutronics software \cite{romano_openmc:_2015}. The condensed neutron energy spectrum forms two
discrete groups bounded at $10^{-5}$, $10^0$, and $10^8$ eV.

I solved for the neutron flux in this system using the following set of numerical solvers:
%
\begin{enumerate}
  \item Neutron diffusion solver with $P_1$-based flux-limited diffusion
    coefficients generated directly from the group constant generation step with OpenMC.
...
diffusion-\gls{SVDC} solvers with a fixed mesh size of 0.005 cm. I omited the OpenMC flux solution
because this exercise aims demonstrate the effectiveness of \glspl{SVDC}
...
Both $P_1$-based diffusion coefficients and \glspl{SVDC} in Figure \ref{fig:c0diffcoef} agree
closely in the bulk fuel salt region far away from the control rod region.
This observation supports the validity of the neutron diffusion method under the four conditions
...
Thus far, this method is trivial and completely redundant because it requires a priori knowledge of
a reference flux solution or at least a highly accurate solution calculated using neutron
...
high-level $S_N$ neutron transport calculation and a low-level neutron diffusion calculation. In
this case, the required number of neutron transport simulations scales with number of reactor
simulations.

The nature of \glspl{SVDC} as empirical, pointwise corrections for the diffusion equation makes it
highly dependent on the neutron flux gradient and susceptible to more significant variations than
region-wide estimates for $P_1$-based diffusion
...
Variations in $P_1$ diffusion coefficients primarily arise from changes in the neutron energy
spectrum with no direct contribution from proximity to material interfaces (geometrical
heterogeneity). Therefore, \gls{SVDC} generation cannot be adopted as one-off preprocessing step.

Another significant challenge of \glspl{SVDC} and the high-order empirical diffusion coefficients
involves their resolution near neutron flux peaks. The neutron currents and flux gradients in the
numerator and denominator of Eq. \ref{eq:svdc} generally do not go to zero at the same point,
resulting in diffusion coefficient values tending to positive or negative infinity when the
flux gradient is close or equal to zero. Pounders \& Rahnema avoided this issue by using
...
computational cost savings by treating most of the reactor geometry with the neutron
diffusion method alone. Henceforth, I will accordingly refer to the $S_N$ calculation on the correction region
as the $S_N$ \textit{subproblem} or \textit{subsolver}. I define the entire problem
...
\end{align}
%
In the absence of additional information, I assume the next best guess, which is a uniformly
...
neutron fluxes. However, the influence of boundary conditions on $J$ and $\sfrac{d\phi}{dx}$ does
not extend far from the boundaries in optically thick media. By taking the ratio of $J$
to $\sfrac{d\phi}{dx}$ to calculate \glspl{SVDC}, I eliminate any errors from $\phi$ since both
$J$ and $\sfrac{d\phi}{dx}$ scale approximately linearly with $\phi$. The \glspl{SVDC}
...
In this subsection, I revisit Case 0, the 1-D two-region system described in Section \ref{sec:svdc}
(Figure \ref{fig:case-0-geom}), to test my hypothesis and demonstrate the Hybrid $S_N$-Diffusion
method. Once again, I defer general numerical implementation details to
...
Refer to Figures \ref{fig:c0g1hd} and \ref{fig:c0g2hd} for a qualitative comparison of
group 1 and 2 $P_1$-based diffusion coefficients, \glspl{SVDC} derived from the reference $S_8$
calculation as demonstrated in Section \ref{sec:svdc}, and \glspl{SVDC} derived from the Hybrid
$S_N$-Diffusion method discussed here. The \glspl{SVDC} from the hybrid method agree well
with the reference \glspl{SVDC} in the control rod region and for most of the fuel-graphite region
up to around $x=15$ cm. Both sets of \glspl{SVDC} approximately coincide with the $P_1$ diffusion
coefficient values within 1\% tolerance from $x=14.615$ cm and $x=3.390$ cm onwards for group 1 and
...
this work with up to the 2nd Legendre moments of the scattering cross sections.
...
Given that the $S_N$ subsolver only covers $\Omega^d_1$, it does not calculate an independent $k$
...
The ceonvergence of the results depends mainly on the following simulation parameters: mesh size
...
\begin{table}[tb!]
  ...
  \begin{tabular}{c S}
    ...
    \multirow{2}{*}{\textbf{Method}} & {\textbf{Control rod worth}} \\
                                     & {$\bm{\rho_{(5b)}-\rho_{(5a)}}$} \\
    \midrule
    OpenMC-CE & 0.56549(280) \\
    OpenMC-MG & 0.55464(240) \\
    $S_8$     & 0.55507 \\
    Diffusion & 0.57246 \\
    Hybrid    & 0.54974 \\
    \bottomrule
...
I also developed an algorithm for identifying and discarding inaccurate \glspl{SVDC} values, which
